...
module.exports = function(Poot) {
  ...
  Poot.getconfig = function(pootid, cb) {
    var response = {field: pootid}
      Poot.find({pootid: pootid}, {include: 'speurpunt'}, function(err, result){
      console.log(result);
      });
    cb(null, response);
  };
  ...
  Poot.getSpeurpunt = function(cb){
    //todo create unique integer
    console.log('in get speurpunt')
    Poot.find({fields:{pootid: true, id: false}}, function(err, poten){
      if(err) console.log(err);
      var ids = poten.map(function(x){return x.pootid})
      var pootid = ids.reduce(function(a,b){return Math.max(a,b);})
